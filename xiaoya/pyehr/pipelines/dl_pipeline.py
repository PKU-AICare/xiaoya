...
class DlPipeline(L.LightningModule):
    def __init__(self, config):
        ...
        self.task = config["task"] if "task" in config else "multitask"
        self.los_info = config["los_info"] if "los_info" in config else None
        self.model_name = config["model"] if "model" in config else "GRU"
        self.main_metric = config["main_metric"] if "main_metric" in config else "auprc"
