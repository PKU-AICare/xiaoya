...
class TGRU(nn.Module):
    def __init__(self, input_dim, hidden_dim: int=32, feat_dim: int=8, act_layer=nn.GELU, drop=0.0, **kwargs):
        ...
        self.mhsa = nn.MultiheadAttention(feat_dim, self.num_heads, dropout=drop, batch_first=True)
    ...
    def forward(self, x, **kwargs):
        ...
        for i, gru in enumerate(self.grus):
            ...
            attn = self.mhsa(cur_feat, cur_feat, cur_feat, average_attn_weights=True)
            attention[:, :, i] = attn[0]
